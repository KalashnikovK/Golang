...
func main() {
	// Все это не нужно объявлять для Степика. В окно Степика нужно вставить только от сих
	...
	for i := 0; i < len(pos); i += 2 {
		workArray[pos[i]], workArray[pos[i+1]] = workArray[pos[i+1]], workArray[pos[i]]
	}
	for idx := range workArray {
		fmt.Print(workArray[idx], " ")
	}
	// до сих, без функции main
}
